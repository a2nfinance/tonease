import {
    Button, ButtonGroup, Input, InputGroup,
    InputLeftAddon, Modal, ModalBody,
...
export default function WithdrawModal() {
    ...
    const handleWithdrawPaymentRequest = useCallback((amount: number) => {
        ...
        dispatch(setShowWithdrawModal(false));
        dispatch(withdrawPaymentRequestThunk({amount, tonConnectUi}));

    }, [amount, wallet])

    return (
        <Modal isOpen={showWithdrawModal} onClose={handleClose} size={{base: "xs", md: 'md'}}>
            <ModalOverlay />
            <ModalContent>
